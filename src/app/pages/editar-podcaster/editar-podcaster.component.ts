    ...
    <div class="min-h-screen bg-black text-white pt-8">
      ...
      <div class="bg-[var(--sponge)] text-white px-8 py-4 flex items-center justify-between mb-8 h-[92px]">
        <div class="flex items-center gap-4">
          <img src="assets/podcasters.png" class="w-8 h-8" alt="Icono artistas" />
      ...
      <div class="flex flex-col items-center justify-center bg-black text-white p-6">
        ...
        <form (ngSubmit)="guardarPodcaster()" #podcasterForm="ngForm" class="flex flex-col items-center gap-6">
          ...
          <div class="flex gap-4 mt-4">
            ...
            <button type="submit" [disabled]="!podcasterForm.form.valid"
      ...
      <div *ngIf="mostrarModal" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
        <div class="bg-black text-white p-6 rounded-2xl border-2 border-[var(--sponge)] w-[90%] max-w-md shadow-xl text-center animate-fade-in">
          <h2 class="text-xl font-semibold mb-4">¿Eliminar podcaster?</h2>
...
export class EditarPodcasterComponent {
  nombre: string = '';
  biografia: string = '';
  ...
  ngOnInit() {
    const nombrePodcaster = this.route.snapshot.paramMap.get('nombre');
    if (nombrePodcaster) {
      this.authService.getPodcaster(nombrePodcaster).subscribe((data) => {
        this.nombre = data.nombre_podcaster;
  ...
  confirmarEliminacion() {
    this.authService.eliminarCreador(this.nombre).subscribe({
      ...
      error: (error) => {
        ...
        alert('Hubo un error al intentar eliminar el podcaster.');
      }
    });
