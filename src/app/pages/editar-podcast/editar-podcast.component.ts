  ...
  template: `
    ...
    <div *ngIf="mostrarModal" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div class="bg-black text-white p-6 rounded-2xl border-2 border-[var(--sponge)] w-[90%] max-w-md shadow-xl text-center animate-fade-in">
        <h2 class="text-xl font-semibold mb-4">¿Eliminar pódcast?</h2>
        <p class="mb-6">¿Estás seguro de que deseas eliminar el pódcast <strong>{{ nombre }}</strong>? Esta acción no se puede deshacer.</p>
...
export class EditarPodcastComponent {
  ...
  ngOnInit() {
    ...
    console.log('Usuario obtenido:', usuario.nombre_usuario);

    const idPodcast = Number(this.route.snapshot.paramMap.get('id_podcast'));
    if (idPodcast) {
      this.authService.getPodcast(idPodcast, this.nombreUsuario).subscribe((data: any) => {
        console.log(data);
        const podcast = data.podcast;
        this.idPodcast = podcast.id_podcast; 
        this.nombre = podcast.nombre_podcast; 
        this.descripcion = podcast.descripcion;
        this.previewImage = podcast.link_imagen; 
        this.tematicas = podcast.tematicas.join(', '); // Convertir el array "tematicas" en un string separado por comas
        // Convertir el array "creadores" en un string separado por comas
  ...
  guardarPodcast() {
    ...
    formData.append('id_podcast', this.idPodcast.toString());
    ...
    this.authService.updatePodcast(formData).subscribe({
      next: () => {
        alert('Cambios guardados correctamente');
        this.router.navigate(['/admin/gestionar-podcasts']);
      },
      error: (error) => {
        console.error('Error al guardar los cambios del pódcast:', error);
  ...
  confirmarEliminacion() {
    this.authService.eliminarPodcast(this.idPodcast).subscribe({
      next: () => {
        alert('Pódcast eliminado correctamente.');
        this.router.navigate(['/admin/gestionar-podcasts']);
      },
      error: (error) => {
        console.error('Error al eliminar el pódcast:', error);
        alert('Hubo un error al intentar eliminar el pódcast.');
      }
    });
