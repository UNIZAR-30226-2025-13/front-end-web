  ...
  template: `
    <div class="min-h-screen bg-black text-white pt-8">
      ...
      <div class="flex flex-col items-center justify-center bg-black text-white p-6">
        <h2 class="text-2xl font-bold mb-6">NUEVO PODCASTER</h2>

        <form (ngSubmit)="guardarPodcaster()" #podcasterForm="ngForm" class="flex flex-col items-center gap-6">
          ...
          <div class="flex gap-4 mt-4">
            <button type="button" (click)="cancelar()" class="bg-[var(--button)] hover:bg-[var(--buttonhover)] text-white py-2 px-6 rounded-full">
              Cancelar
            </button>
            <button type="submit" [disabled]="!podcasterForm.form.valid || !imagenSeleccionada"
              class="bg-[var(--sponge)] hover:bg-[var(--lightSponge)] text-white py-2 px-6 rounded-full font-semibold">
              Guardar
            </button>
          </div>
        </form>
...
export class NuevoPodcasterComponent {
  ...
  guardarPodcaster() {
    if (!this.nombre || !this.biografia || !this.imagenSeleccionada) return;

    const formData = new FormData();
    formData.append('nombre_creador', this.nombre);
    formData.append('biografia', this.biografia);
    formData.append('es_podcaster', 'true');
    ...
    this.authService.uploadCreador(formData).subscribe({
      ...
      error: (err) => {
        ...
        alert('Ocurrió un error al guardar el podcaster');
      }
    });
  }
